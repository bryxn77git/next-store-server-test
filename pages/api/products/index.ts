...
const getProducts = async(req: NextApiRequest, res: NextApiResponse<Data>) => {
    ...
    if( req.query.brands !== 'all' ){
        ...
        if (typeof arrBrands === 'string') {
            condition = { ...condition, marca: { $in: arrBrands.split(',')}}
        }
    }

    if( req.query.sizes !== 'all' ){
        const arrSizes = req.query.sizes
        if (typeof arrSizes === 'string') {
            condition = { ...condition, tallas: { $in: arrSizes.split(',')}}
        }
    }

    if( req.query.colors !== 'all' ){
        const arrColors = req.query.colors
        if (typeof arrColors === 'string') {
            condition = { ...condition, 'colores.nombre': { $in: arrColors.split(',') }}
        }
    }
    

    if( query !== undefined ){
        const sortUrl = query.split('=')[1];
        switch (sortUrl) {
            case 'a-z':
                sort = { title: 'asc' };
                break;
    ...
    await db.connect();
    const products = await Product.find({ $or: [condition]})
        .sort(sort)
