...
const Categoria = () => {
  ...
  const handleShowBrand = (e: any, checked: boolean) => {
    ...
    if(checked){
        if( !brandsSelected.find(v => v === e.target.labels[0].innerText)){
            if( brandsSelected.includes('all') ){
              setBrandsSelected([e.target.labels[0].innerText.toLowerCase()]);

            }else{
              setBrandsSelected([...brandsSelected, e.target.labels[0].innerText.toLowerCase()]);
        ...
        if(brandsSelected.length === 1){
            ...
            let arr = brandsSelected.filter(v => v !== e.target.labels[0].innerText.toLowerCase())
    ...
    const handleShowSize = (e: any, checked: boolean) => {
      ...
      if(checked){
        if( !sizesSelected.find(v => v === e.target.labels[0].innerText)){
          if( sizesSelected.includes('all') ){
            setSizesSelected([e.target.labels[0].innerText]);
          }else{
  ...
  const changeStateColor = ( position: number, colorAdded: { 
    ...
    if(!checked){
        if( !colorSelected.find( color => color === colorAdded.nombre)){
          if( colorSelected.includes('all') ){
            setColorSelected([colorAdded.nombre]);
  ...
  return (
    <ShopLayout title={category.charAt(0).toLocaleUpperCase() + category.slice(1).toLowerCase() } pageDescription={`Encuentra los mejores productos de ${category}`} >
